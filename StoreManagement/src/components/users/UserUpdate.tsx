...
export const UserUpdate = () => {
  ...
  const handleUpdate = async (event: { preventDefault: () => void }) => {
    ...
    try {
      ...
      console.log(error);
      openSnackbar("error", "Failed to update user due to an unknown error!");
    }
  };
        ...
        <Card sx={{ p: 2 }}>
          <CardContent>
            <Box display="flex" alignItems="flex-start" sx={{ mb: 4 }}>
              <IconButton component={Link} sx={{ mb: 2, mr: 3 }} to={`/users`}>
                <ArrowBackIcon />
              </IconButton>
              <h1
                style={{
            ...
            <form>
              ...
              <FormControl fullWidth>
                ...
                >
                  {getEnumValues(AccessLevel).map((accessLevelValue) => (
                    <MenuItem key={accessLevelValue} value={accessLevelValue}>
