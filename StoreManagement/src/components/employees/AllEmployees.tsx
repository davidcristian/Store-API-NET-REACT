...
import { getAccount, isAuthorized, useAuthToken } from "../../auth";
...
export const AllEmployees = () => {
  ...
  useEffect(() => {
    ...
    const sortedCurrentUsers = currentEmployees.sort((a, b) => {
      const aVal = a[sorting.key as keyof Employee];
      const bVal = b[sorting.key as keyof Employee];

      console.log("a", aVal, typeof aVal);
      console.log("b", bVal, typeof bVal);
  ...
  return (
    <Container>
      ...
      {!loading && employees.length === 0 && (
        <p data-testid="test-all-employees-empty" style={{ marginLeft: 16 }}>
          No employees found.
        </p>
      )}
      {!loading &&
              ...
              <Grid item xs={12} sm={6} md={4} key={employee.id}>
                <Card>
                  <CardContent>
                    ...
                    <Typography color="text.secondary">
                      # of Shifts: {employee.storeShifts?.length}
                    </Typography>
                  </CardContent>
                  <CardActions>
                    <IconButton
                  ...
                  {headers.map((header, i) => {
                    ...
                    return (
                      <TableCell
                        key={i}
                        style={{
                          cursor: header.propName ? "pointer" : "default",
                          whiteSpace: header.propName ? "nowrap" : "normal",
                          userSelect: "none",
                        }}
